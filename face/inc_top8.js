...
function loadReports(reportType, reportURL, email, lang, action, level, onlyFavourites, source, source_id, ip) {
  loadAjaxToDiv(level + '/includes/ajax_report.jsp?report_type=' + reportType + '&reportURL=' + reportURL + '&email=' + email + '&action=' + action + '&lang=' + lang + '&level=' + level + "&source=" + source + "&source_id=" + source_id + "&ip=" + ip, "report-confirm");
}
  ...
  xmlHttpReportPrint.onreadystatechange = function() {
      ...
      if(xmlHttpReportPrint.status == 200) {
            ...
            if (exportTarget == 0) {
                ...
                if (userAgent.indexOf("Firefox") >= 0) {appName = "Firefox";}
                
                if (appName == 'Microsoft Internet Explorer') {
                    window.location = windowName;
                } else 
                if (appName == 'Netscape') {  // safari, crome
