    ...
    xmlHttpAjax22.onreadystatechange = function () {
        if (xmlHttpAjax22.readyState == 4) {
            if (xmlHttpAjax22.status == 200) {
                ...
                window.location=level+"/index.jsp";
...
function historyBackToState(idHistory) {
    loadAjaxToDiv('../includes/ajax_history_back_state.jsp?history_id=' + idHistory, 'none');
    ...
    xmlHttpAjax.onreadystatechange = function () {
        if (xmlHttpAjax.readyState == 4) {
            if (xmlHttpAjax.status == 200) {
                ...
                sendmail_init(source,source_id,email,'', xmlHttpAjax.responseText.replace(/^\s+|\s+$/g, ""));
...
function loadReportMailNameFromSessionPortal(idReport, email, source_id, source) {
    var xmlHttpAjax;
    if (window.ActiveXObject) {
        xmlHttpAjax = new ActiveXObject("Microsoft.XMLHTTP");
    }
    ...
    xmlHttpAjax.onreadystatechange = function () {
        if (xmlHttpAjax.readyState == 4) {
            if (xmlHttpAjax.status == 200) {
                doReportAid(idReport, 2, source);
                sendmail_init(source,source_id,email,'', xmlHttpAjax.responseText.replace(/^\s+|\s+$/g, ""),true);
                //popUpMail("../mail/sendmail.jsp?fileNameShort=" + xmlHttpAjax.responseText.replace(/^\s+|\s+$/g, "") + "&mail_to=" + email + "&reportSourceId=" + source_id + "&setSubject=true&source=" + source);
            }
        }
    }
...
function loadReportMailPortal(url, idReport, email, source_id, source) {
    var xmlHttpAjax;
    if (window.ActiveXObject) {
        xmlHttpAjax = new ActiveXObject("Microsoft.XMLHTTP");
    }
    ...
    xmlHttpAjax.onreadystatechange = function () {
        if (xmlHttpAjax.readyState == 4) {
            if (xmlHttpAjax.status == 200) {
                loadReportMailNameFromSessionPortal(idReport, email, source_id, source);
            }
        }
    }
    xmlHttpAjax.open("GET", url, true);
        ...
        if (xmlHttpReportPrint.readyState == 4) {
            if (xmlHttpReportPrint.status == 200) {
                ...
                if (exportTarget == 0) {
                    ...
                    loadReportMailPortal(xmlHttpReportPrint.responseText.replace(/^\s+|\s+$/g, "") + '&exportType=' + exportType + '&exportTarget=' + 1, idReport, email, source_id, source);
...
function setOpenPortal(link,source,source_id) {
    window.openPortal = function openPortal() {
...
function setDocumentByReport(document, idDocument) {
    ...
    if (document == "invoice") {
        ...
        if (getElementChecked("chb_ordered_items")) {
            var5 = "T";
        }
        doit = true;
    }
...
function saveLibCountryFromChoose(country_name, country_iso, country_currency_id, country_language_id, 
    country_international_code, variable) {
    ...
    if (country_name != '') {
        ...
        xmlHttpAjax.onreadystatechange = function() {
            if (xmlHttpAjax.readyState == 4) {
                if (xmlHttpAjax.status == 200) {
                    var idCountry = xmlHttpAjax.responseText.replace(/^\s+|\s+$/g, "");
                    updateChooseValue(variable, country_name, idCountry);
                    showHideFinstatSKButton();
                    showHideAresCZButton();
                    //doAutoIcdph();
                }
            }
        }
        xmlHttpAjax.open("GET", "../lib/ajax/save_country.jsp?country_name=" + country_name + 
                "&country_iso=" + country_iso + 
                "&country_international_code=" + country_international_code + 
                "&country_lcurrency_id=" + country_currency_id + 
                "&country_language_id=" + country_language_id, true);
        xmlHttpAjax.send(null);
    }
}